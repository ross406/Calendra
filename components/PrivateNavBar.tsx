...
export default function PrivateNavBar() {
    ...
    return (
        <nav className="flex justify-between items-center fixed z-50 w-full h-28 bg-gray-200 px-10 gap-4 shadow-2xl mb-28">
              {/* Logo */}
              <Link href="/events" className="flex items-center gap-1 hover:scale-115 duration-500 ">
                <Image
                  ...
                  {PrivateNavLinks.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        key={item.label}
                        className={
                          cn('flex gap-4 items-center p-4 rounded-lg justify-start hover:scale-115 duration-300 ',
              ...
              {/* User button */}
              <div className='hover:scale-115 duration-500 '>
                <SignedIn>
                    {/* Mount the UserButton component */}
                    <UserButton 
                      appearance={{
                      elements: {
                        userButtonAvatarBox: 'my-custom-avatar', // increase avatar size
                      },
                    }} />
                </SignedIn>
